///*** Configuration Start ***///
///Styling
var fontawesome = window.fontawesome = false;
var ficon = window.ficon = ' <i class="fa fa-external-link" style="font-size: 10px;vertical-align: middle;"></i> ';
///Protected Keywords Of Links
...
/* Safelink Converter By Dimas Lanjaka
 * 2018 Copyrighted
 * Coder Name : Dimas Lanjaka
 * Alias : L3n4r0x
 * Website : https://web-manajemen.blogspot.com
 * GitHub : https://www.github.com/dimaslanjaka
 * Codepen : https://codepen.io/dimaslanjaka
 * Facebook : https://www.facebook.com/dimaslanjaka
 * Fanspage : https://www.facebook.com/secretnetworkforces
 * YouTube : https://www.youtube.com/channel/UCGNaoefvJRfd15fo-
 * LICENSE MIT V.0.8
 */
...
function v2() {
  ...
  for (var i = 0; i < linktag.length; i++) {
    var enc = aesCrypto.encrypt(convertstr(linktag[i].href), convertstr('root'));
    var hrf = AESf + enc;
    var href = linktag[i].href;
    if (href.match(protectedLinks) === null) {
      linktag[i].href = hrf;
      linktag[i].target = "_blank";
      if (fontawesome == true) {
        linktag[i].innerHTML = linktag[i].innerHTML + ficon;
      }
    }
  }
}
...
function ExEc() {
  var arc = [1, 2];
  var item_arc = arc[Math.floor(Math.random() * arc.length)];
  return eval('v' + item_arc + '()');
}
...
function v1() {
  if (window.jQuery) {
    $('a').each(function() {
      if (this.href.match(protectedLinks)) {} else {
        $(this).attr("href", safelink + btoa(this.href));
        $(this).attr("target", "_blank");
        if (fontawesome == true) {
          $(this).html($(this).html() + ficon);
        }
      }
    });
    ...
    for (var i = 0; i < l.length; i++) {
      ...
      if (!href.match(protectedLinks)) {
        l[i].setAttribute("href", safelink + btoa(href));
        l[i].setAttribute("target", "_blank");
        if (fontawesome == true) {
          l[i].innerHTML = l[i].innerHTML + ficon;
        }
      }
    }
  } else if (document.getElementsByTagName('a')) {
